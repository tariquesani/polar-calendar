...
for i in range(12):
    ...
    ax.plot([angle, angle], [start_time, end_time], color='#02735E', linewidth=0.5, zorder=10)
...
for j, day_index in enumerate(sundays):
    ...
    cumulative_days = np.cumsum(days_in_month)
    month_index = next((i for i, total in enumerate(cumulative_days) if day_index < total), 11)
    ...
    rotation = (rotation + 180) % 360 - 180  # Normalize rotation for proper alignment
    ha = 'center'
...
hour_labels_to_display = [
    '4:00AM', '4:15AM','4:30AM', '4:45AM','5:00AM', '5:15AM','5:30AM','5:45AM', '6:00AM','6:15AM', '6:30AM','6:45AM'
]
hour_ticks_to_display = [
    4 / 24, 4.25 / 24, 4.5 / 24, 4.75 / 24, 5 / 24, 5.25 / 24, 5.5 / 24,5.75 / 24, 6 / 24,6.25 / 24, 6.5 / 24, 6.75 / 24, 7 / 24
]
...
for i, label in enumerate(hour_labels_to_display):
    ...
    ax.text(
        np.pi / 2,  # Choose an appropriate angle for label placement
        radius,
        label,
        ha='left',
        va='center',
...
for tick in hour_ticks_to_display:
    ax.fill_between(theta, tick - 0.0001, tick + 0.0001, color='gray', alpha=0.4, zorder=3, linewidth=0)
